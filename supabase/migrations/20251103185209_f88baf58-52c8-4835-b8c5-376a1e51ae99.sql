-- Create dev_projects table (types already exist)
...
CREATE TABLE IF NOT EXISTS public.dev_sprints (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS public.dev_builds (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID REFERENCES public.dev_projects(id) ON DELETE CASCADE NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dev_task_time_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  task_id UUID REFERENCES public.dev_tasks(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dev_project_attachments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID REFERENCES public.dev_projects(id) ON DELETE CASCADE NOT NULL,
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_projects' AND policyname = 'Admins can view all dev projects'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_projects' AND policyname = 'Admins can manage dev projects'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_sprints' AND policyname = 'Admins can view all sprints'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_sprints' AND policyname = 'Admins can manage sprints'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_tasks' AND policyname = 'Admins can view all dev tasks'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_tasks' AND policyname = 'Admins can manage dev tasks'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_builds' AND policyname = 'Admins can view all builds'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_builds' AND policyname = 'Admins can manage builds'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_task_comments' AND policyname = 'Admins can view all task comments'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_task_comments' AND policyname = 'Admins can manage task comments'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_task_time_logs' AND policyname = 'Admins can view all time logs'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_task_time_logs' AND policyname = 'Admins can manage time logs'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_project_attachments' AND policyname = 'Admins can view all dev attachments'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'dev_project_attachments' AND policyname = 'Admins can manage dev attachments'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_dev_projects_client_id ON public.dev_projects(client_id);
...
CREATE INDEX IF NOT EXISTS idx_dev_sprints_project_id ON public.dev_sprints(project_id);
CREATE INDEX IF NOT EXISTS idx_dev_sprints_status ON public.dev_sprints(status);

CREATE INDEX IF NOT EXISTS idx_dev_tasks_project_id ON public.dev_tasks(project_id);
CREATE INDEX IF NOT EXISTS idx_dev_tasks_sprint_id ON public.dev_tasks(sprint_id);
...
CREATE INDEX IF NOT EXISTS idx_dev_tasks_parent_task_id ON public.dev_tasks(parent_task_id);

CREATE INDEX IF NOT EXISTS idx_dev_builds_project_id ON public.dev_builds(project_id);
CREATE INDEX IF NOT EXISTS idx_dev_builds_environment ON public.dev_builds(environment);

CREATE INDEX IF NOT EXISTS idx_dev_task_comments_task_id ON public.dev_task_comments(task_id);
CREATE INDEX IF NOT EXISTS idx_dev_task_comments_user_id ON public.dev_task_comments(user_id);

CREATE INDEX IF NOT EXISTS idx_dev_task_time_logs_task_id ON public.dev_task_time_logs(task_id);
CREATE INDEX IF NOT EXISTS idx_dev_task_time_logs_user_id ON public.dev_task_time_logs(user_id);

CREATE INDEX IF NOT EXISTS idx_dev_project_attachments_project_id ON public.dev_project_attachments(project_id);
CREATE INDEX IF NOT EXISTS idx_dev_project_attachments_task_id ON public.dev_project_attachments(task_id);

-- Create triggers for updated_at timestamps (drop first if exists)
...
DROP TRIGGER IF EXISTS update_dev_sprints_updated_at ON public.dev_sprints;
CREATE TRIGGER update_dev_sprints_updated_at
  BEFORE UPDATE ON public.dev_sprints
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS update_dev_tasks_updated_at ON public.dev_tasks;
CREATE TRIGGER update_dev_tasks_updated_at
  BEFORE UPDATE ON public.dev_tasks
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS update_dev_task_comments_updated_at ON public.dev_task_comments;
...
-- Storage RLS policies (admin-only) - use DO blocks to avoid errors if policies exist
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' AND policyname = 'Admins can upload dev attachments'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' AND policyname = 'Admins can view dev attachments'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' AND policyname = 'Admins can update dev attachments'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' AND policyname = 'Admins can delete dev attachments'
  ) THEN
